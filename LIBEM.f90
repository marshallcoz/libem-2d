!  Variable modules
      ...
      logical, parameter    :: makeVideo = .false.
...
!     use gloVars, only: dp
      ...
      !  G_{i,m} (x,xi)  cuado este punto es xi: 
      ...
      ! allpoints           | | | | 
      complex*16, dimension(:,:,:,:), allocatable :: FKhGre,FKvGre
      ...
      !                     | | | ,--- x: 1...NPts (receptores)
      ! BouPoints           | | | | 
      complex*16, dimension(:,:,:,:), allocatable :: FKhTre,FKvTre
      
      
       end type Punto   
      ! bondary elements:     ,--- POINT index
      ...
      integer, save :: nIpts, nMpts, nBpts, NPts,&
...
!     stop 0
      ...
      integer :: I,m,iP,iPxi,iP_x,direction !small loop counter
      integer :: status
...
!     print*,'b ',bptini,'-',bptfin
      ...
      if (makeVideo) deallocate(moviePoints)
...
!       COME=COME*(UR - UI/2.0/Qq) !histeretic damping
       ...
       ! Free field solution on the FK plane:
       !(only at points in the same stratum as the application force)
       call FreeFieldAtPoints(xfsource,zfsource,efsource,0,J,COME,PrintNum) 
       
      if (workBoundary) then
      do iPxi = 1,nBpts
       call FreeFieldAtPoints(fixedBouPoints(iPxi)%center(1), & 
                              fixedBouPoints(iPxi)%center(2), & 
                              fixedBouPoints(iPxi)%layer, & 
                              iPxi,J,COME,PrintNum) 
      end do
      end if
...
!para ibem
       ...
       ! de colocacion xi y el resto de los puntos, incluso ellos mismos.
        ...
        call vectorB_force(fixedBouPoints(ip)%center(1), & 
                           fixedBouPoints(ip)%center(2), & 
                           fixedBouPoints(ip)%layer, & 
                           fixedBouPoints(ip)%isOnInterface, & 
                           direction,cOME,k)
        Bk = B
        Ak = A
        IPIV = 4*N+2
...
!     call FreeFieldAtPoints(xfsource,zfsource,efsource,J,COME,PrintNum) 
 ...
 ! IBEM
      ...
      allocate(ibemMat(nBpts,nBpts))
      allocate(trac0vec(nBpts))
      allocate(ibemPHI(nBpts,2,2))
 ...
 ! fill the matrix by rows
      ...
      ! una columna para cada fuerza puntual 
      
      do i=1,2 ! componente de desplazamiento en el receptor
      do m=1,2 ! componente de fuerza en fuente puntual
      
      dO iP_x=1,nBpts !receptor (renglon)
      dO iPxi=1,nBpts !fuente virtual (columna) 
         if (iPxi .eq. iP_x) then
            ibemMat(iP_x,iPxi) = 0.5
         else
         !                            receptor---,       ,--- fuentes
         !                                    ___|__ ____|_
!           ibemMat(iP_x,iPxi) = integralEq14(iP_x,i,iPxi,m,J,ik)
  ...
  ! fill vector free field at receptors
         if (i .eq. 1) then
            trac0vec(iP_x) = & 
            (allpoints(iP_x+bPtini-1)%FKh(ik,J,5)* nxfsource + &
            allpoints(iP_x+bPtini-1)%FKv(ik,J,5)* nzfsource) * &
            allpoints(iP_x+bPtini-1)%normal(1) + &
            (allpoints(iP_x+bPtini-1)%FKh(ik,J,4)* nxfsource + &
            allpoints(iP_x+bPtini-1)%FKv(ik,J,4)* nzfsource) * &
            allpoints(iP_x+bPtini-1)%normal(2)
         ...
         elseif (i .eq. 2) then
            ...
            allpoints(iP_x+bPtini-1)%FKv(ik,J,4)* nzfsource) * &
            allpoints(iP_x+bPtini-1)%normal(1) + &
            (allpoints(iP_x+bPtini-1)%FKh(ik,J,3)* nxfsource + &
            allpoints(iP_x+bPtini-1)%FKv(ik,J,3)* nzfsource) * &
            allpoints(iP_x+bPtini-1)%normal(2)
            
         end if
      end dO !iP_x (renglon)
  ! system is built, now solve phi for current i,j
!     ibemPHI(:,i,m)
      IPIVibem = nBpts
      call zgesv(nBpts,1,ibemMat,nBpts,IPIVibem,trac0vec,nBpts,info)
        if(info .ne. 0)then
          write(PrintNum,'(a)')"Problem with ibem system"
          write(PrintNum,'(a,I10)')"INfO: ",info
          stop 0
        end if
      ibemPHI(:,i,m) = trac0vec
      end do ! m
      end do ! i 
  ! stuff all the results into an FK plane of diffracted stuff
...
!       end if!
        ...
        "Number of verical movie nodes: ", nMpts
        iP = 1
        do iz = 1,nz
...
!              moviePoints(1)%center(iP,2),"]"
      ...
      integer :: iXI,iX,e,indice, AllocateStatus,idx
      real :: XIx, XIy
...
!     end do
      ...
      do iXI=1,nBpts !      xi                                  x
        allocate(BouPoints(iXI)%FKhTre(nmax+1,Nfrec+1,imecMax,Npts))
        allocate(BouPoints(iXI)%FKvTre(nmax+1,Nfrec+1,imecMax,Npts))
      end do
!     
      do iX=1,nPts !        x                                  xi
        allocate(allpoints(iX)%FKhGre(nmax+1,Nfrec+1,imecMax,NBpts))
        allocate(allpoints(iX)%FKvGre(nmax+1,Nfrec+1,imecMax,NBpts))
      end do
      ...
      real, dimension(:), allocatable :: auxVector
      real :: nuevoPx,deltaX,deltaZ,maxLen,leniXI
      integer :: J,iXI,e,indice, AllocateStatus,idx
      real :: XIx, XIy
      character(LEN=100) :: txt
      real     :: errT = 0.001
      logical, dimension(:), allocatable  :: isOnIF
...
!     
      do iX=1,nPts 
        deallocate(allpoints(ix)%FKhGre,allpoints(ix)%FKvGre)
                   !        x                                  xi
        allocate(allpoints(iX)%FKhGre(nmax+1,Nfrec+1,imecMax,NBpts))
        allocate(allpoints(iX)%FKvGre(nmax+1,Nfrec+1,imecMax,NBpts))
      end do
...
!     ! ................................................................
      ...
      subroutine FreeFieldAtPoints(x_f,z_f,e,iP_xi,nJ,cOME,outpf)
      use soilVars !                           `--- (0): allpoints
      use gloVars  !                            (iP_xi): fixedBouPoints
...
!                            e => efsource
      ...
      integer                    :: ik,iP
      real                       :: k
      real, dimension(NPts)      :: x_i,z_i,SGNz
!     real                       :: p,q
!     complex*16                 :: k
!     complex*16, dimension(3,2) :: subMatSB
      
      complex*16, dimension(NMAX+1,NPts) :: G11,G33,G31, & 
                                          S333,S313,S113,s331,s131,s111
      complex*16, dimension(NPts) :: egamz,enuz
      real                       :: L2M
      ...
      if (iP_xi .eq. 0) then !the case when we the source is the source 
       ...
       do iP = 1,Npts !to solve the ibem matrix
...
!       print*,'[',x_i,',',z_i,'] ',e
      ...
      use gloVars, only : imecMax
      use resultVars, only : MecaElem,allpoints,BouPoints,NPts
      ...
      ! ya estamos dentro de un loop en puntos xi
      do iP=1,NPts ! punto x (receptor)
        x_i = allpoints(iP)%center(1)
        z_i = allpoints(iP)%center(2)
          e = allpoints(iP)%layer
        this_Meca = calcMecaAt_xi_zi(iP,x_i,z_i,e,cOME_i,outpf)
        ...
        if (direction .eq. 1) then !x
          BouPoints(iP_xi)%FKhGre(iik,nJ,1:imecMax,iP) = &
          BouPoints(iP_xi)%FKhGre(iik,nJ,1:imecMax,iP) + & 
          this_Meca%Rw(1:imecMax)
        elseif (direction .eq. 2) then !z
          BouPoints(iP_xi)%FKvGre(iik,nJ,1:imecMax,iP) = &
          BouPoints(iP_xi)%FKvGre(iik,nJ,1:imecMax,iP) + & 
          this_Meca%Rw(1:imecMax)
        end if
...
!     end subroutine vectorB_incidence
      subroutine filterFK(preguntar,tt,xAx,yAx,outpf)
      use DISLIN
      use waveNumVars, only : NFREC,NMAX,DFREC,DK
      use glovars
      use resultvars!, only: allpoints,NPts
      ...
      !                     | | ,--- iMec: 1:5
      !                     | | | ,--- iP: de puntos
!     complex*16,dimension(NMAX,NFREC+1,imecMax,nsize) :: thisFK
!     complex*16,dimension(NMAX,NFREC+1,imecMax) :: testFK
...
!     real, dimension(nsize,2) :: coords
!     real, dimension(1,2) :: testCoords
!     real :: x0,y0
      character(LEN=10), intent(in) :: tt
...
!        testFK = allpoints(2)%FK !el segundo de interestingPoints.txt
!        x_i = allpoints(2)%center(1) 
!        z_i = allpoints(2)%center(2)
!        testCoords(1,1) = x_i; testCoords(1,2) = z_i
...
!        print *, char(7)
      ...
      ! We are pleased apply to the rest of the points
         
         do iP=1,NPts
         do ik=1,NMAX*2
         do iMec = 1,imecMax
      allpoints(iP)%FK(ik,:,iMec) = allpoints(iP)%FK(ik,:,iMec) * Hf
         end do
         end do
         !
         do ifr=1,NFREC+1
         do iMec = 1,imecMax
      allpoints(iP)%FK(:,ifr,iMec) = allpoints(iP)%FK(:,ifr,iMec) * HK
         end do
         end do
         end do
       
      end subroutine filterFK
